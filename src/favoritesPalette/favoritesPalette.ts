...
export interface PaletteOptions {
   ...
   * worker reads them and hands them over as data URIs.
   */
  icons?: FaviconMap;
  /**
...
export function openFavoritesPalette({
  ...
  /**
   * Acts on a row: a page is somewhere to go, a folder is somewhere to look.
   * Picking a folder retypes the search as that folder's name, which is how its
   * contents come up — and keeps the palette open, because you are still
   * choosing.
   */
  function openRow(index: number, event: MouseEvent | KeyboardEvent) {
    ...
    if (target.kind === 'folder') {
      search.value = `.${target.folder.title}`;
      rebuild();
    ...
    close();
    onOpenPage(target.page.url, wantsNewTab(event));
  }
  ...
  function requestWidenedSearch() {
    ...
    void searchAllBookmarks(parsed.term).then((results) => {
      ...
      if (search.value === asked) {
        widened = { bookmarks: results.bookmarks, folders: results.folders };
        allIcons = { ...allIcons, ...results.icons };
        rebuild();
      }
    });
  }
