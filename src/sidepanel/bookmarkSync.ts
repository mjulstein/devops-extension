...
import type { FoldedBookmark, WidenedSearchData } from './favoritesListing';
import {
  bookmarksInFolder,
...
export async function searchAllBookmarks(
  term: string,
  limit = 40
): Promise<WidenedSearchData> {
  if (!chrome.bookmarks?.search || !chrome.bookmarks.getTree) {
    return { bookmarks: [], folders: [] };
  }

  const tree = await chrome.bookmarks.getTree();
  const folders = matchBookmarkFolders(collectBookmarkFolders(tree), term);

  // Nothing typed means every bookmark, which is not an answer — the folders
  // above are. Once something is typed, a folder whose name matches brings its
  // contents along with whatever matched by title or address.
  if (!term.trim()) {
    return { bookmarks: [], folders };
  }

  const fromFolders = folders.flatMap((folder) =>
    bookmarksInFolder(tree, folder.id).map((entry) => ({
      url: entry.url,
      title: entry.title,
      folder: entry.folder
    }))
  ...
  const foldered: FoldedBookmark[] = [];

  for (const entry of fromFolders) {
  ...
  const parentTitles = await readFolderTitles(kept);
  ...
  return {
    ...
    bookmarks: [
      ...
      ...kept.map((entry) => ({
        page: { url: entry.url, label: entry.title, starredAt: 0 },
        folder: entry.parentId ? (parentTitles.get(entry.parentId) ?? '') : ''
      }))
    ]
  };
}
