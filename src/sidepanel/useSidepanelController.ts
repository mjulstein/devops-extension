...
export function useSidepanelController() {
  ...
  const recentFeatureSuggestions = useMemo(
    () => getSuggestionsByGroup(parentSuggestions, 'feature'),
    [parentSuggestions]
  );

  const recentParentableSuggestions = useMemo(
    () => getSuggestionsByGroup(parentSuggestions, 'parentable'),
    [parentSuggestions]
  );
  ...
  }, [isActiveItemPinned, pinnedActiveWorkItemContext, activeWorkItemContext]);
  ...
  async function onToggleRecentFeaturesCollapsed() {
    ...
    await saveRecentFeaturesCollapsed(nextValue);
  }
