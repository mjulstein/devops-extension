...
- `ThemeSwitch.tsx` + `ThemeSwitch.module.css` — the light/dark switch on the panel's title bar, where it reads as acting on the whole panel rather than on the controls beside it. It shows and toggles Azure DevOps's own theme, and is disabled when there is no connection to read or change it with, since a control that silently does nothing is worse than a dead one.
