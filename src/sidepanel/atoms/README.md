...
- `ThemeSwitch.tsx` + `ThemeSwitch.module.css` — the light/dark switch at the panel's top right. It shows and toggles Azure DevOps's own theme, and is disabled when there is no connection to read or change it with, since a control that silently does nothing is worse than a dead one.
