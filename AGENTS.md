...
- `src/bookmarks.html` + `src/bookmarks.tsx` + `src/bookmarks/*` — the extension's own bookmark manager page, which the palette's **Bookmarks** button opens in place of the browser's. The browser's manager cannot show what is saved twice or what is now empty, which is most of the reason it gets opened; it opens as a single popup window rather than a tab (`openSingletonWindow` in the service worker raises the one already open and closes any extra, and remembers the window's bounds); the page is three hand-sizable columns, drag-and-drop organiser and **Duplicate name** / **Duplicate path** / **Duplicate folder** / **Empty folders** lists, and every row carries edit, delete and move-to-folder in place. Folder rows carry collapse, rename, delete and flatten, with a confirmation before deleting a folder that is not empty; flattening lifts the contents into the grandparent at the folder's own position and drops the empty shell, so only the level named disappears. Selecting a folder in the tree scopes all four lists to that subtree, since a duplicate matters where you are working; clicking a row in a list instead reveals its folder in the tree without rescoping, which would otherwise drop the counterpart being compared. Row checkboxes form one bulk selection across both halves, with move and delete over the whole set. Duplicate *path* compares urls without their search params and only reports copies in different folders, since two in one folder are already visible side by side; duplicate *folder* rows rename and move but never delete, because such a folder may be full. See [`src/bookmarks/README.md`](./src/bookmarks/README.md)
- `src/sidepanel.html` — side panel HTML entry
- `src/sidepanel.tsx` — React side panel entry
- `src/sidepanel/atoms/PopOutPanelButton.tsx` — moves the panel into a window of its own, through the same `openSingletonWindow` the bookmark manager uses: one at a time, a popup rather than a tab so no tab strip or toolbar comes with it, and its bounds remembered. The service worker owns the window rather than the panel that asked, since that panel may be the one the browser closes on the way
...
- `src/palette.html` + `src/palette.tsx` — the favorites palette as a window of its own, which is what the shortcut opens now. A window reaches every page rather than only the ones a content script runs on, and takes focus by being a window rather than by asking. It closes on another browser window taking focus (never on the browser itself going to the background, which happens on every glance at another application) and after an idle timeout, both cancelled by the pin in its header; the pin lives in the worker's memory, since it describes the window open right now. `FAVORITES_SEARCH_SURFACE` in the service worker switches back to the overlay below
